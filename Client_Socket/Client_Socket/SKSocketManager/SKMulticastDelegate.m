...
@interface SKMulticastDelegateNode : NSObject {
@private
...
@interface SKMulticastDelegateEnumerator ()
{
    NSUInteger numNodes;
...
@implementation SKMulticastDelegateNode
...
@implementation SKMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    SKMulticastDelegateNode *node =
    [[SKMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    {
        SKMulticastDelegateNode *node = delegateNodes[i - 1];
...
{
    for (SKMulticastDelegateNode *node in delegateNodes)
    {
        node.delegate = nil;
#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
        node.unsafeDelegate = nil;
...
- (SKMulticastDelegateEnumerator *)delegateEnumerator
{
    return [[SKMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation SKMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
    ...
    {
        SKMulticastDelegateNode *node = delegateNodes[currentNodeIndex];
        currentNodeIndex++;
